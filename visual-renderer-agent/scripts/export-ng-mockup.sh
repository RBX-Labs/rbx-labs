...
TARGET_PAGE="$ROOT_DIR/removed product.html"
TMP_IMAGE="/tmp/removed product-mockup-full.png"
TMP_CAPTURE_HTML="$ROOT_DIR/.removed product-capture-static.html"

mkdir -p "$MOCKUP_DIR"

TODAY="$(date '+%Y-%m-%d')"
CANONICAL="$MOCKUP_DIR/removed product-mockup-desktop.png"
VERSIONED="$MOCKUP_DIR/removed product-mockup-desktop-$TODAY.png"
CANONICAL_FULL="$MOCKUP_DIR/removed product-mockup-desktop-full.png"
VERSIONED_FULL="$MOCKUP_DIR/removed product-mockup-desktop-full-$TODAY.png"
...
if [[ -f "$OUT_DIR/removed product-desktop.html.png" ]]; then
  cp "$OUT_DIR/removed product-desktop.html.png" "$CANONICAL"
  cp "$OUT_DIR/removed product-desktop.html.png" "$VERSIONED"
fi
